...
import static org.qubership.atp.itf.lite.backend.model.api.ApiPath.REQUEST_PARAMS_PATH;
...
@ContextConfiguration(classes = {RequestParamsController.class})
@MockBean(RequestParamService.class)
class RequestParamsControllerTest extends AbstractControllerTest {

    @Test
    void disableParamParam() throws Exception {
        final UUID paramId = UUID.randomUUID();
        final UUID projectId = UUID.randomUUID();
        final UUID requestId = UUID.randomUUID();
        this.mockMvc.perform(put(SERVICE_API_V1_PATH + REQUEST_PARAMS_PATH + ID_PATH + DISABLE_PATH, paramId)
    ...
    @Test
    void enableRequestParam() throws Exception {
        final UUID paramId = UUID.randomUUID();
        final UUID projectId = UUID.randomUUID();
        final UUID requestId = UUID.randomUUID();
        this.mockMvc.perform(put(SERVICE_API_V1_PATH + REQUEST_PARAMS_PATH + ID_PATH + ENABLE_PATH, paramId)
                .param("projectId", projectId.toString())
                .param("requestId", requestId.toString())
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON))
